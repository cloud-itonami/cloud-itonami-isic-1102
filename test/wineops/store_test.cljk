...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:product-type :wine/still-table :abv-percent 12.5}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:product-type :wine/still-table}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:product-type :wine/still-table :abv-percent 12.5}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:product-type (get-in result [:batches "batch-001"])) :wine/still-table))
      (is (= (:abv-percent (get-in result [:batches "batch-001"])) 12.5)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:product-type :wine/still-table}}}
